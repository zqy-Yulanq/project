...
def lanzhou_chart3():
    ...
    sql = "SELECT id, y_test FROM lanzhou_real_predicted limit 100,200"
    cur.execute(sql)
    array1 = [list(x) for x in cur.fetchall()]
    data = ([array1])
    # Get id=100-200 predicted house prices for the test set
    sql2 = "SELECT id, prediction FROM lanzhou_real_predicted limit 100,200"
    cur.execute(sql2)
    array2 = [list(x) for x in cur.fetchall()]
    print(array2)
    # Combine two arrays into one array
...
@app.route('/lanzhou_chart4/', methods=['POST'])
@login_required
def lanzhou_chart4():
    conn = pymysql.connect(host='127.0.0.1', user='root', password='root', db='housing', charset='utf8')
    cur = conn.cursor()
    sql = "SELECT district,ROUND(AVG(price),4)'Avarage Price' FROM lanzhou_housing_all group by district"
    cur.execute(sql)
    array = [list(x) for x in cur.fetchall()]
    data = []
    for x in array:
        mydict = {'name': x[0], 'value': x[1]}
        data.append(mydict)
    conn.close()
    print(data)
    return json.dumps(data)
...
def lanzhou_add_prediction():
    if flask.request.method == 'GET':
        ...
        basepath = os.path.dirname(__file__)
        # Note: If there is no folder, you must create it first, or you will be prompted that there is no such path
        upload_path = os.path.join(basepath, 'rf_reg.pkl')
        estimator = joblib.load(upload_path)
        # Normalize the data to be predicted and the data in the test set
        transfer = StandardScaler()
        prediction_array = transfer.fit_transform(prediction_array)
